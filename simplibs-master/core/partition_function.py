...
from scipy import *
import pandas as pd
from core.fundamental_constants import *
...
def partition_function(element, ion_state,Tev):
    ...
    #read from data
    df = pd.read_csv('core/database/partition_data/' +element+" "+ ion_state+'.txt',delimiter=',',index_col=False)
    ...
    for i in range(0,len(energies)):
        ...
        ZZ+=degeneracies[i]*exp(-energies[i]/(kb*Tev*T_ref))
        #print(ZZ)
...
def test_partition_function():
    ...
    element_list=array(df['Element'])
    element_number = arange(1,len(array(df['Element']))+1)

    #set the ionized state for each element - max_ion_state defines a maximum
    max_ion_state = 4
    element_ions=[]
    for i in range(0,len(element_number)):
        element_ions.append(arange(0,min([element_number[i],max_ion_state])))
    ...
    for i1 in range(0,len(element_list)):
        ...
        for j1 in range(0,len(element_ions[i1])):
            ...
            current_file = open('core/database/partition_data/' +element+" "+ ion_state+'.txt','r')
